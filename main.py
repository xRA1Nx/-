        ...
        for p3 in temp_list2:
            ...
            for p4 in temp_list3:
                ...
                for p5 in temp_list4:
                    ...
                    distance_dict[str(p5) + str(p1)] = ((p5[0] - p1[0]) ** 2 + (p5[1] - p1[1]) ** 2) ** 0.5
                    ...
                    res_dict[f"{p1} -> {p2}[{p1p2}] -> {p3}[{p2p3}] -> {p4}[{p3p4}] -> {p5}[{p4p5}] -> {p1}[{p5p1}] ="] = p5p1
    #print(len(res_dict))
...
def greed_way(inp_list):
    start_point = inp_list[0]
    point1 = start_point
    txt = f"{start_point}"
    dist = 0

    greed_list = inp_list[1:].copy()
    ...
    for p in greed_list:
        ...
        txt += f" -> {p}[{dist}]"
        point1 = p
    dist += ((point1[0] - start_point[0]) ** 2 + (point1[1] - start_point[1]) ** 2) ** 0.5
